...
class RocccBramInStream(Stream):
    ...
    def get_adapter_instance(self):
        ...
        return burst.get_adapter_instance() + [
                "%s_stream : input_bram_wrapper" % self.full_name,
                "generic map(",
                "\tINPUT_DATA_WIDTH => 32,",
                "\tOUTPUT_DATA_WIDTH => %s_data_channel0'length" % self.full_name,
                ")",
                "port map(",
                "\tclk           => clk,",
                "\trst           => rst,",
                '\tbram_en       => bram_app_m2s(%d).re,'  % (self.memory.index),
                '\tbram_addr     => bram_app_m2s(%d).addr_r,' % (self.memory.index),
                '\tbram_dout     => bram_app_s2m(%d).dout,' % (self.memory.index),
                "\tfull          => %s_Full," % self.full_name,
                "\tdata_ready    => %s_writeEn," % self.full_name,
                "\tdata          => %s_stream_channel," % self.full_name,
                "\taddress       => %s_stream_address," % self.full_name,
                "\taddress_ready => %s_address_translator_address_valid" % self.full_name,
                ");",
...
class RocccBramOutStream(Stream):
    ...
    def get_adapter_instance(self):
        ...
        return burst.get_adapter_instance() + fifo.get_adapter_instance() + syncInterface.get_adapter_instance() + [
                "%s_stream : output_bram_wrapper" % self.full_name,
                "port map(",
                "\tclk           => clk,",
                "\trst           => %s_output_rst," % (self.instance.name),
                '\tbram_en       => bram_app_m2s(%d).we,'  % (self.memory.index),
                '\tbram_addr     => bram_app_m2s(%d).addr_w,' % (self.memory.index),
                '\tbram_din      => bram_app_m2s(%d).din,' % (self.memory.index),
                '\t  done_in => %s_done,' % self.full_name,
                '\t  empty_in => %s_0_empty,' % self.full_name,
                '\t  read_enable_out => %s_0_stream_read_enable,' % self.full_name,
                '\t  channel_in => %s_0_mc_data,' % self.full_name,
                '\t  address_in => %s_0_address_fifo_data_2,' % self.full_name,
                '\t  read_in => %s_0_read' % self.full_name,
                ');',
...
class BramStream(BramBaseStream):
    ...
    def get_adapter_instance(self):
        if self.in_stream and self.out_stream:
            ...
            'bram_app_m2s(%d).addr_r <= %s_addrb;' % (self.memory.index, self.name),
            'bram_app_m2s(%d).addr_w <= %s_addra;' % (self.memory.index, self.name),
            'bram_app_m2s(%d).din <=  %s_dina;' % (self.memory.index, self.name),
            '%s_doutb <= bram_app_s2m(%d).dout;' % (self.name, self.memory.index),
            'mem_app_m2s(%d) <= ADB3_OCP_DEFAULT_M2S;' % self.memory.index
            ]
